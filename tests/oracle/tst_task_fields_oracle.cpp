...
void TstTaskFieldsOracle::fieldsMatchXml()
{
    ...
    int n = 0, idOk = 0, olOk = 0, pcOk = 0, msOk = 0, smOk = 0;
    ...
    qInfo().noquote() << QFileInfo(mpp).fileName()
                      << QStringLiteral("ID %1  Outline %2  Pct %3  Milestone %4  Summary %5  (of %6)")
                             .arg(idOk).arg(olOk).arg(pcOk).arg(msOk).arg(smOk).arg(n);
