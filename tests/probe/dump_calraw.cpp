...
//
// Diagnostic: dump the raw TBkndCal stream quartet so ScheduleIO's calendar
// records can be byte-compared against real MS Project ground truth.
// usage: dump_calraw <file.mpp>
...
int main(int argc, char **argv)
{
    ...
    if (argc < 2) { std::printf("usage: dump_calraw <file.mpp>\n"); return 2; }
    ...
    const QString dir = QStringLiteral("   114"), sub = QStringLiteral("TBkndCal");
    ...
    constexpr int kMetaItem = 10, kRec = 12, kF2MetaItem = 10;
    ...
    for (int i = 0; i < items; ++i) {
        ...
        if (off + kRec <= quint32(fd.size())) {
            std::printf("        rec: base=0x%08x res=0x%08x uid=%u\n",
                        u32(fd, int(off)), u32(fd, int(off) + 4), u32(fd, int(off) + 8));
        }
    }
    ...
    for (int i = 0; i < f2items; ++i) {
        ...
        if (off + 48 <= quint32(f2d.size())) {
            std::printf("        f2d first24: ");
            hexLine(f2d, int(off), 24);
        }
    }
